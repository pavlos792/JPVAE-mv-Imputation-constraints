...
class OrthogMat(nn.Module):
    ...
    def __call__(self, x):
        #print(f"DEBUG: OrthogMat __call__ input x shape: {x.shape}")
        # Orthogonalize the eigenvectors (e.g., using QR decomposition)
        # q, r = jnp.linalg.qr(self.eigenvectors)
        # # Create diagonal matrix from eigenvalues
        # Lambda = jnp.diag(self.eigenvalues)
        ...
        output_call = jnp.matmul(O, x)
        #print(f"DEBUG: OrthogMat __call__ O_matrix shape: {O.shape}, output shape: {output_call.shape}")
        return output_call
    ...
    def get_orthogonal_matrix(self):
        ...
        A = U - jnp.transpose(U)
        O = jnp.matmul(jnp.eye(self.num_evecs) + A, jnp.linalg.inv(jnp.eye(self.num_evecs) - A))
        #print(f"DEBUG: OrthogMat.get_orthogonal_matrix() returning O_matrix shape: {O.shape}")
        return O
...
class Encoder1(nn.Module):
 ...
 @nn.compact
 def __call__(self, x):
      #print(f"DEBUG: Encoder input x shape: {x.shape}")
      ...
      #print(f"DEBUG: Encoder output mu shape: {mean_x.shape}, logvar shape: {logvar_x.shape}")
...
class Decoder1(nn.Module):
  ...
  def __call__(self, z,apply_sigmoid = True):
      #print(f"DEBUG: Decoder input z shape: {z.shape}")
      ...
      #print(f"DEBUG: Decoder output recon_x shape: {logits.shape}")
      #return logits
      if apply_sigmoid:
...
#  latents2: int = 20
 ...
 def __call__(self, x1, x2, z_rng):
   #print(f"\nDEBUG: VAE __call__ - x1 shape: {x1.shape}, x2 shape: {x2.shape}")
  ...
  #  mean2, logvar2 = self.encoder2(x)
   ...
   #print(f"DEBUG: VAE __call__ - z1 shape (from x1): {z1.shape}")
   z2 = reparameterize(rngs[1], mean2, logvar2)
   #print(f"DEBUG: VAE __call__ - z2 shape (from x1): {z2.shape}")
   logits2 = self.decoder2(z2)
   logits1 = self.decoder1(z1)
   mat = self.mat(self.alpha*jnp.eye(self.latents[0]))
   #print(f"DEBUG: VAE __call__ - OrthogMat 'mat' output shape: {mat.shape}")
 ...
 def cross_gen(self, z1_input, z2_input, rng): 
    ...
    # recreating the O matrix from its internal parameters
    # The get_orthogonal_matrix method in OrthogMat is designed for this.
    O_matrix = self.mat.get_orthogonal_matrix()
    #print(f"DEBUG: cross_gen - Retrieved O_matrix shape: {O_matrix.shape}")
    ...
    #print(f"DEBUG: cross_gen - z2_predicted_from_z1 shape: {z2_predicted_from_z1.shape}")
    #print(f"DEBUG: cross_gen - recon_x2_cond_gen shape: {recon_x2_cond_gen.shape}")
    ...
    #print(f"DEBUG: cross_gen - z1_predicted_from_z2 shape: {z1_predicted_from_z2.shape}")
    #print(f"DEBUG: cross_gen - recon_x1_cond_gen shape: {recon_x1_cond_gen.shape}")
...
def cond_mean(z2, z1, rng):
    ...
    mean_vec1 = jnp.mean(z1, axis=0)
    mean_vec2 = jnp.mean(z2, axis=0)
