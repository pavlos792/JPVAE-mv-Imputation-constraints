...
import training_steps_evals as ts 
...
def eval(params, images1, images2, z_1, z_2, z_rng, latents, num_out, alpha, binary,param, zero=False, print_images=False, h=14, w=28, results_dir=None, figures_dir=None):
  ...
  def eval_model(vae):
    loss = []
    for i in range(50):
      recon_images1, mean1, logvar1, z1, recon_images2, mean2, logvar2, z2, U,S, V = vae(images1, images2, rngs[i])
      mat = jnp.matmul(U,jnp.matmul(jnp.diag(S),V))
    ...
    recon_images1, mean1, logvar1, z1, recon_images2, mean2, logvar2, z2, U,S,V = vae(images1, images2, rngs[0])
    mat = jnp.matmul(U,jnp.matmul(jnp.diag(S),V))
    if zero:
...
def eval_generation(params, images1, images2, z_1, z_2, main_rng, latents, num_out, alpha, binary,param, zero=False, print_images=False, h=14, w=28, results_dir=None, figures_dir=None, mask_rng=None):
  #rngs = random.split(z_rng, 50)
  #n_images = images1.shape[0]
  #mask_choices = random.bernoulli(mask_rng, p=0.5, shape=(n_images,))
  ...
  def eval_model(vae):
    ...
    for i in range(50):
      ...
      recon_x1_dummy, mean1, logvar1, z1, recon_x2_dummy, mean2, logvar2, z2, U, S, V = vae.apply(
          {'params': params}, input_x1_masked, input_x2_masked, rngs[i]
      )
      
      # Conditional generation using vae_model_instance.cross_gen
      ...
      mat_for_metrics = jnp.matmul(U, jnp.matmul(jnp.diag(S), V))
    ...
    _, _, _, _, _, _, _, _, U_final, S_final, V_final = vae.apply({'params': params}, input_x1_masked, input_x2_masked, rngs[0])
    
    final_mat = jnp.matmul(U_final, jnp.matmul(jnp.diag(S_final), V_final))
...
def get_z(params, images1, images2, z_rng, latents,num_out, alpha):
  def eval_model(vae):
    recon_images1, mean1, logvar1, z1, recon_images2, mean2, logvar2, z2, _,_,_= vae(images1, images2, z_rng)
...
def get_all_z(params, images1, images2, z_rng, latents, num_out, alpha, binary):
  def eval_model(vae):
    recon_images1, mean1, _, z1, recon_images2, mean2, _, z2, _, _, _= vae(images1, images2, z_rng)
...
def train_and_eval_split(config: ml_collections.ConfigDict, train_ds,val_ds, test_ds, full_train,full_val,
  ...
  for epoch in range(config.num_epochs):
    ...
    metrics_val, comparison_val, sample1_val, sample2_val, ll_val, _, _, mat_val= eval_f(
        state.params, val_ds1, val_ds2, z_1, z_2, eval_rng, config.latents, config.num_out, config.alpha,  binary, param, zero, print_images = True,
